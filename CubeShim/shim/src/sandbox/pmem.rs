...
/// Shared guest kernel module image: every `.ko` shipped next to a guest
/// vmlinux lives in here, `cube_gauge.ko` among them.
pub const DEFAULT_GAUGE_PMEM_PATH: &str =
    "/usr/local/services/cubetoolbox/cube-kernel-scf/cube_modules.ext4";
...
impl Pmem {
    ...
    /// Host path of `cube_modules.ext4`. Prefer the toolbox plane file; if that
    ...
    pub fn gauge_pmem_path(kernel: &str) -> String {
        ...
        if let Some(parent) = Path::new(kernel).parent() {
            let sibling = parent.join("cube_modules.ext4");
            if sibling.is_file() {
