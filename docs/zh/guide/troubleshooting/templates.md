...
| 加了 `--enable-metric` 但容器里 `/proc/modules` 没有 `cube_gauge` | 开关必须在做模板时打开。`insmod` 在做该模板时、用户容器起来之后执行，不是 restore 时。看做模板那次沙箱 id 的 Shim 日志是否有 `ask agent to insmod`；`/run/cube-gauge` 在 agent 命名空间，不在用户容器里。 | [客户机性能指标（GAUGE）](../guest-perf-metrics.md) |
