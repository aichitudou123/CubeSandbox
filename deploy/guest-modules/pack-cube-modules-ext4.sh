#!/usr/bin/env bash
# Build the guest kernel modules against KDIR (the guest kernel *tree*, not a
# vmlinux ELF) and pack them into a 2MiB no-journal ext4. Every guest-side .ko
# rides in this one image; today that is cube_gauge.ko alone.
#
# Required:
...
log() { printf '[cube-modules] %s\n' "$*"; }
die() { printf '[cube-modules] ERROR: %s\n' "$*" >&2; exit 1; }
...
truncate -s "${IMAGE_BYTES}" "${work}/cube_modules.ext4"
mkfs.ext4 -F -b 4096 -O ^has_journal -d "${work}/root" "${work}/cube_modules.ext4" >/dev/null
mkdir -p "$(dirname "${OUTPUT}")"
cp -f "${work}/cube_modules.ext4" "${OUTPUT}"
...
meta = {
    ...
    "cube_modules_ext4_md5": ext4_md5,
    "vermagic": vermagic,
    "kdir": kdir,
}
