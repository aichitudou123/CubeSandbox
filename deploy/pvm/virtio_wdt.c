...
static int virtio_watchdog_ping(struct watchdog_device *wdd)
{
	...
	if (*buf != 1) {
		dev_err(&vdev->dev, "Host did not acknowledge buffer correctly");
		ret = -EINVAL;
	}
err:
	vi->busy = false;
...
static int probe_common(struct virtio_device *vdev)
{
	...
	vi->index = index = ida_simple_get(&watchdog_index_ida, 0, 0, GFP_KERNEL);
	if (index < 0) {
		err = index;
		goto err_ida;
	}
	...
	vi->vq = virtio_find_single_vq(vdev, virtio_watchdog_recv_done, "input");
	if (IS_ERR(vi->vq)) {
		err = PTR_ERR(vi->vq);
		goto err_find;
	}
